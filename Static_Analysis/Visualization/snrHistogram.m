...
for a = 1:nChannels
    ...
    stepSNR = getStepSNR(gridData, channel, minSteps, maxSteps);

    binwidth = (max(stepSNR)-min(stepSNR)) / 100;
    bins = [min(stepSNR):binwidth:max(stepSNR)];
    figure;
    bar(bins,histc(stepSNR,bins));
...
disp(['Median SNR for this dataset is ' num2str(median(stepSNR)) ]);